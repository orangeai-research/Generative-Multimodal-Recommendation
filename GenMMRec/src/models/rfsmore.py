...
r"""
RFSMORE: RF-Enhanced SMORE
Integrates Rectified Flow module to enhance spectrum-based modality representation fusion
...
class RFSMORE(SMORE):
    ...
    def forward(self, adj, train=False):
        ...
        if self.use_rf and train:
            ...
            # Visual features (spectrum convolved)
            if self.v_feat is not None:
                conditions.append(image_embeds.detach())
            
            # Text features (spectrum convolved)
            if self.t_feat is not None:
                conditions.append(text_embeds.detach())
            
            # Fusion features (cross-modality fusion)
            conditions.append(fusion_embeds.detach())

            # Compute user prior (spectrum-based deviation)
            if len(conditions) > 0:
            ...
            with torch.no_grad():
                ...
                conditions.append(fusion_embeds)
