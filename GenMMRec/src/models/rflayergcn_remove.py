...
r"""
RFLayerGCN: RF-Enhanced LayerGCN
Integrates Rectified Flow module to enhance graph embeddings
...
class RFLayerGCN(LayerGCN):
    ...
    def pre_epoch_processing(self):
        ...
        # Call parent's pre_epoch_processing for edge dropout
    ...
    def forward(self):
        ...
        if self.use_rf and self.training:
            print(f"[RFLayerGCN] Forward in TRAINING mode")
            ps_loss = 0.0
            if self.use_denoise:
                ...
                if denoised_emb is not None:
                    ...
                    rf_target = ui_all_embeddings.detach()
            else:
                rf_target = ui_all_embeddings.detach()

            # No multimodal conditions for LayerGCN (pure CF model)
            loss_dict = self.rf_generator.compute_loss_and_step(
                target_embeds=rf_target,
                conditions=[],  # No multimodal features
                user_prior=None,  # No prior for simple GCN
            ...
            ui_all_embeddings = self.rf_generator.mix_embeddings(
                ui_all_embeddings,
                rf_embeds.detach(),
                training=True,
                epoch=self.rf_generator.current_epoch,
            ...
            print(f"[RFLayerGCN] Forward in INFERENCE mode")
            # Inference mode
            with torch.no_grad():
                rf_embeds = self.rf_generator.generate([])  # No multimodal conditions
                ui_all_embeddings = self.rf_generator.mix_embeddings(
                    ui_all_embeddings,
                    rf_embeds,
                    training=False,
    ...
    def full_sort_predict(self, interaction):
        ...
        with torch.no_grad():
            if self.use_rf:
                ...
                restore_user_e, restore_item_e = torch.split(ui_all_embeddings, [self.n_users, self.n_items])
            else:
